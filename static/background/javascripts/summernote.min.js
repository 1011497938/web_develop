!function(a) {
    "use strict";
    var b = navigator.appVersion.indexOf("Mac")>-1, c = navigator.userAgent.indexOf("MSIE")>-1, d = function() {
        var a = function(a) {
            return function(b) {
                return a === b
            }
        ...
        var b = function(a) {
        ...
        }, q = b("#text"), r = function(a) {
            return q(a) ? a.nodeValue.length : a.childNodes.length
        }, s = function(a) {
            ...
            return a = o(a.cloneNode(!1), a), p(a, n(c))
        }, w = function(a, b, c) {
            var e = j(b, d.eq(a));
            return 1 === e.length ? v(b, c) : e.reduce(function(a, d) {
                var e = d.cloneNode(!1);
                return o(e, d), a === b && (a = v(a, c)), p(e, n(a)), e
            })
        }, x = function(a, b) {
            if (a && a.parentNode) {
                if (a.removeNode)
                ...
                var c = a.parentNode;
                if (!b) {
                    for (var d = [], e = 0, f = a.childNodes.length; f > e; e++)
                        d.push(a.childNodes[e]);
                    for (var e = 0, f = d.length; f > e; e++)
                        c.insertBefore(d[e], a)
                    }
                c.removeChild(a)
            }
        };
        ...
        }, e = ["bold", "italic", "underline", "strikethrough", "justifyLeft", "justifyCenter", "justifyRight", "justifyFull", "insertOrderedList", "insertUnorderedList", "indent", "outdent", "formatBlock", "removeFormat", "backColor", "foreColor", "insertImage", "insertHorizontalRule"], i = 0, j = e.length; j > i; i++)
            this[e[i]] = function(a) {
                return function(b, c) {
        ...
        this.formatBlock = function(a, b) {
            ...
            e({
                ...
                h.select(), d(b);
                var f =- 1 !== e.toLowerCase().indexOf("://"), i = f ? e : "http://" + e;
                if (c && h.isCollapsed()) {
            ...
            var c = JSON.parse(b);
            d(a), document.execCommand("foreColor", !1, c.foreColor), document.execCommand("backColor", !1, c.backColor)
        }, this.insertTable = function(b, e) {
            d(b);
            for (var f, h = e.split("x"), i = h[0], j = h[1], k = [], l = c ? "&nbsp;" : "<br/>", m = 0; i > m; m++)
                k.push("<td>" + l + "</td>");
            f = k.join("");
        ...
        this.update = function(b, c) {
            ...
            if (c.image) {
                var e = a(c.image), f = e.position(), g = {
                    w: e.width(),
                    h: e.height()
                };
                d.css({
        ...
        this.showImageDialog = function(b, c, d) {
            var e = b.find(".note-image-dialog"), f = b.find(".note-dropzone"), g = b.find(".note-image-input");
            e.on("shown.bs.modal", function() {
                ...
                f.off("dragenter dragover dragleave drop"), g.off("change"), e.off("shown.bs.modal hidden.bs.modal")
            }).modal("show")
        }, this.showLinkDialog = function(a, b, c) {
            var d = a.find(".note-link-dialog"), e = d.find(".note-link-text"), f = d.find(".note-link-url"), g = d.find(".note-link-btn");
        ...
        var c = new j, d = new k, e = new l, g = new m, h = new n, i = {
            BACKSPACE: 8,
            TAB: 9,
            ENTER: 13,
            SPACE: 32,
            ...
            var d = b ? a.metaKey: a.ctrlKey, e = a.shiftKey, f = a.keyCode, g = d || e || f === i.TAB, j = g ? o(a.target): null;
            if (f === i.TAB && j.editable().data("tabsize"))
                c.tab(j.editable());
            else if (d && (e && f === i.Z || f === i.Y))
                c.redo(j.editable());
            else if (d && f === i.Z)
                c.undo(j.editable());
            else if (d && f === i.B)
                ...
                c.underline(j.editable());
            else if (d && e && f === i.S)
                c.strikethrough(j.editable());
            else if (d && f === i.BACKSLACH)
                c.removeFormat(j.editable());
            else if (d && f === i.K)
                c.setLinkDialog(j.editable(), function(a, b) {
                    h.showLinkDialog(j.dialog(), a, b)
                });
            else if (d && f === i.SLASH)
                h.showHelpDialog(j.dialog());
            else if (d && e && f === i.L)
                c.justifyLeft(j.editable());
            else if (d && e && f === i.E)
                c.justifyCenter(j.editable());
            else if (d && e && f === i.R)
                c.justifyRight(j.editable());
            else if (d && e && f === i.J)
                c.justifyFull(j.editable());
            else if (d && e && f === i.NUM7)
                c.insertUnorderedList(j.editable());
            else if (d && e && f === i.NUM8)
                c.insertOrderedList(j.editable());
            else if (d && f === i.LEFTBRACKET)
                c.outdent(j.editable());
            else if (d && f === i.RIGHTBRACKET)
                c.indent(j.editable());
            else if (d && f === i.NUM0)
                c.formatBlock(j.editable(), "P");
            else if (d && i.NUM1 <= f && f <= i.NUM6) {
                var k = "H" + String.fromCharCode(f);
                c.formatBlock(j.editable(), k)
            } else {
                if (!d || f !== i.ENTER)
                    return (f === i.BACKSPACE || f === i.ENTER || f === i.SPACE) && c.recordUndo(o(a.target).editable()), void 0;
                c.insertHorizontalRule(j.editable())
            }
            a.preventDefault()
        }, q = function(b, d) {
            b.trigger("focus"), a.each(d, function(a, d) {
            ...
            if (b && b.files) {
                var c = o(a.currentTarget || a.target);
            ...
            var b = o(a.currentTarget || a.target), f = c.currentStyle(a.target);
            f && (d.update(b.toolbar(), f), e.update(b.popover(), f), g.update(b.handle(), f))
        }, u = function(a) {
            var b = o(a.currentTarget || a.target);
            e.hide(b.popover()), g.hide(b.handle())
        }, v = function(b) {
            if (f.isControlSizing(b.target)) {
                var d, h = o(b.target), i = h.handle(), j = h.popover(), k = h.editable(), l = h.editor(), m = i.find(".note-control-selection").data("target"), n = a(m), p = n.offset(), q = a(document).scrollTop();
            ...
            if (e.length > 0) {
                var f, g = e.attr("data-event"), i = e.attr("data-value"), j = o(b.target), k = j.dialog(), l = j.editable();
                if ( - 1 !== a.inArray(g, ["resize", "float"])) {
                    var m = j.handle(), n = m.find(".note-control-selection");
                    f = n.data("target")
                }
                if (c[g] && (l.trigger("focus"), c[g](l, i, f)), - 1 !== a.inArray(g, ["backColor", "foreColor"]))
                    d.updateRecentColor(e[0], g, i);
                else if ("showLinkDialog" === g)
                    c.setLinkDialog(l, function(a, b) {
                        h.showLinkDialog(k, a, b)
                    });
                else if ("showImageDialog" === g)
                    h.showImageDialog(k, r, function(a) {
                        q(l, a)
                    });
                else if ("showHelpDialog" === g)
                    h.showHelpDialog(k);
                else if ("fullscreen" === g) {
                    var p = j.editor();
                    p.toggleClass("fullscreen");
                    var s = j.toolbar(), u = function() {
                        var b = a(window).height() - s.outerHeight();
                        l.css("height", b)
                    }, v = p.hasClass("fullscreen");
                    v ? (l.data("orgHeight", l.css("height")), a(window).resize(u).trigger("resize")) : (l.css("height", l.data("orgHeight")), a(window).off("resize")), d.updateFullscreen(s, v)
                }
                t(b)
            }
        }, y = 24, z = function(b) {
            var c = a(document), d = o(b.target).editable(), e = d.offset().top - c.scrollTop(), f = function(a) {
                d.height(a.clientY - (e + y))
            }, g = function() {
                c.unbind("mousemove", f).unbind("mouseup", g)
            };
            c.mousemove(f).mouseup(g), b.stopPropagation(), b.preventDefault()
        }, A = 18, B = function(b) {
        ...
        this.attach = function(a, b) {
            a.editable.on("keydown", p), a.editable.on("mousedown", s), a.editable.on("keyup mouseup", t), a.editable.on("scroll", u), a.editable.on("dragenter dragover dragleave", !1), a.editable.on("drop", r), a.handle.on("mousedown", v), a.toolbar.on("click", x), a.popover.on("click", x), a.toolbar.on("mousedown", w), a.popover.on("mousedown", w), a.statusbar.on("mousedown", z);
            var c = a.toolbar, d = c.find(".note-dimension-picker-mousecatcher");
            d.on("mousemove", B), b.onenter && a.editable.keypress(function(a) {
                a.keyCode === i.ENTER && b.onenter(a)
            }), b.onfocus && a.editable.focus(b.onfocus), b.onblur && a.editable.blur(b.onblur), b.onkeyup && a.editable.keyup(b.onkeyup), b.onkeydown && a.editable.keydown(b.onkeydown)
        }, this.dettach = function(a) {
            a.editable.off(), a.toolbar.off(), a.handle.off(), a.popover.off()
        }
    }, p = function() {
        var c = {
            picture: '<button type="button" class="btn btn-default-outline btn-sm btn-small" title="插入图片" data-event="showImageDialog" tabindex="-1"><i class="icon-picture"></i></button>',
            link: '<button type="button" class="btn btn-default-outline btn-sm btn-small" title="插入链接" data-event="showLinkDialog" data-shortcut="Ctrl+K" data-mac-shortcut="⌘+K" tabindex="-1"><i class="icon-link"></i></button>',
            table: '<button type="button" class="btn btn-default-outline btn-sm btn-small dropdown-toggle" title="插入表格" data-toggle="dropdown" tabindex="-1"><i class="icon-table"></i> <span class="caret"></span></button><ul class="dropdown-menu"><div class="note-dimension-picker"><div class="note-dimension-picker-mousecatcher" data-event="insertTable" data-value="1x1"></div><div class="note-dimension-picker-highlighted"></div><div class="note-dimension-picker-unhighlighted"></div></div><div class="note-dimension-display"> 1 x 1 </div></ul>',
            style: '<button type="button" class="btn btn-default-outline btn-sm btn-small dropdown-toggle" title="风格" data-toggle="dropdown" tabindex="-1"><i class="icon-magic"></i> <span class="caret"></span></button><ul class="dropdown-menu"><li><a data-event="formatBlock" data-value="p">正常</a></li><li><a data-event="formatBlock" data-value="blockquote"><blockquote>引用</blockquote></a></li><li><a data-event="formatBlock" data-value="pre">代码</a></li><li><a data-event="formatBlock" data-value="h1"><h1>标题 1</h1></a></li><li><a data-event="formatBlock" data-value="h2"><h2>标题 2</h2></a></li><li><a data-event="formatBlock" data-value="h3"><h3>标题 3</h3></a></li><li><a data-event="formatBlock" data-value="h4"><h4>标题 4</h4></a></li><li><a data-event="formatBlock" data-value="h5"><h5>标题 5</h5></a></li><li><a data-event="formatBlock" data-value="h6"><h6>标题 6</h6></a></li></ul>',
            fontsize: '<button type="button" class="btn btn-default-outline btn-sm btn-small dropdown-toggle" data-toggle="dropdown" title="字号" tabindex="-1"><span class="note-current-fontsize">11</span> <b class="caret"></b></button><ul class="dropdown-menu"><li><a data-event="fontSize" data-value="8"><i class="icon-ok"></i> 8</a></li><li><a data-event="fontSize" data-value="9"><i class="icon-ok"></i> 9</a></li><li><a data-event="fontSize" data-value="10"><i class="icon-ok"></i> 10</a></li><li><a data-event="fontSize" data-value="11"><i class="icon-ok"></i> 11</a></li><li><a data-event="fontSize" data-value="12"><i class="icon-ok"></i> 12</a></li><li><a data-event="fontSize" data-value="14"><i class="icon-ok"></i> 14</a></li><li><a data-event="fontSize" data-value="18"><i class="icon-ok"></i> 18</a></li><li><a data-event="fontSize" data-value="24"><i class="icon-ok"></i> 24</a></li><li><a data-event="fontSize" data-value="36"><i class="icon-ok"></i> 36</a></li></ul>',
            color: '<button type="button" class="btn btn-default-outline btn-sm btn-small note-recent-color" title="颜色" data-event="color" data-value=\'{"foreColor":"black","backColor":"yellow"}\' tabindex="-1"><i class="icon-font" style="color:black;background-color:yellow;"></i></button><button type="button" class="btn btn-default-outline btn-sm btn-small dropdown-toggle"  data-toggle="dropdown" tabindex="-1"><span class="caret"></span></button><ul class="dropdown-menu"><li><div class="btn-group"><div class="note-palette-title">背景颜色</div><div class="note-color-palette" data-target-event="backColor"></div></div><div class="btn-group"><div class="note-palette-title">字体颜色</div><div class="note-color-palette" data-target-event="foreColor"></div></div></li></ul>',
            bold: '<button type="button" class="btn btn-default-outline btn-sm btn-small" title="加粗" data-shortcut="Ctrl+B" data-mac-shortcut="⌘+B" data-event="bold" tabindex="-1"><i class="icon-bold"></i></button>',
            italic: '<button type="button" class="btn btn-default-outline btn-sm btn-small" title="倾斜" data-shortcut="Ctrl+I" data-mac-shortcut="⌘+I" data-event="italic" tabindex="-1"><i class="icon-italic"></i></button>',
            underline: '<button type="button" class="btn btn-default-outline btn-sm btn-small" title="下划线" data-shortcut="Ctrl+U" data-mac-shortcut="⌘+U" data-event="underline" tabindex="-1"><i class="icon-underline"></i></button>',
            clear: '<button type="button" class="btn btn-default-outline btn-sm btn-small" title="去除格式" data-shortcut="Ctrl+\\" data-mac-shortcut="⌘+\\" data-event="removeFormat" tabindex="-1"><i class="icon-eraser"></i></button>',
            ul: '<button type="button" class="btn btn-default-outline btn-sm btn-small" title="无序列表" data-shortcut="Ctrl+Shift+8" data-mac-shortcut="⌘+⇧+7" data-event="insertUnorderedList" tabindex="-1"><i class="icon-list-ul"></i></button>',
            ol: '<button type="button" class="btn btn-default-outline btn-sm btn-small" title="有序列表" data-shortcut="Ctrl+Shift+7" data-mac-shortcut="⌘+⇧+8" data-event="insertOrderedList" tabindex="-1"><i class="icon-list-ol"></i></button>',
            paragraph: '<button type="button" class="btn btn-default-outline btn-sm btn-small dropdown-toggle" title="对齐" data-toggle="dropdown" tabindex="-1"><i class="icon-align-left"></i>  <span class="caret"></span></button><ul class="dropdown-menu"><li><div class="note-align btn-group"><button type="button" class="btn btn-default-outline btn-sm btn-small" data-event="justifyLeft" tabindex="-1"><i class="icon-align-left"></i></button><button type="button" class="btn btn-default-outline btn-sm btn-small" data-event="justifyCenter" tabindex="-1"><i class="icon-align-center"></i></button><button type="button" class="btn btn-default-outline btn-sm btn-small" data-event="justifyRight" tabindex="-1"><i class="icon-align-right"></i></button><button type="button" class="btn btn-default-outline btn-sm btn-small" data-event="justifyFull" tabindex="-1"><i class="icon-align-justify"></i></button></div></li><li><div class="note-list btn-group"><button type="button" class="btn btn-default-outline btn-sm btn-small" data-event="outdent" tabindex="-1"><i class="icon-indent-left"></i></button><button type="button" class="btn btn-default-outline btn-sm btn-small"  data-event="indent" tabindex="-1"><i class="icon-indent-right"></i></button></li></ul>',
            height: '<button type="button" class="btn btn-default-outline btn-sm btn-small dropdown-toggle" data-toggle="dropdown" title="行间距" tabindex="-1"><i class="icon-text-height"></i>&nbsp; <b class="caret"></b></button><ul class="dropdown-menu"><li><a data-event="lineHeight" data-value="1.0"><i class="icon-ok"></i> 1.0</a></li><li><a data-event="lineHeight" data-value="1.2"><i class="icon-ok"></i> 1.2</a></li><li><a data-event="lineHeight" data-value="1.4"><i class="icon-ok"></i> 1.4</a></li><li><a data-event="lineHeight" data-value="1.5"><i class="icon-ok"></i> 1.5</a></li><li><a data-event="lineHeight" data-value="1.6"><i class="icon-ok"></i> 1.6</a></li><li><a data-event="lineHeight" data-value="1.8"><i class="icon-ok"></i> 1.8</a></li><li><a data-event="lineHeight" data-value="2.0"><i class="icon-ok"></i> 2.0</a></li><li><a data-event="lineHeight" data-value="3.0"><i class="icon-ok"></i> 3.0</a></li></ul>',
            help: '<button type="button" class="btn btn-default-outline btn-sm btn-small" title="Help" data-shortcut="Ctrl+/" data-mac-shortcut="⌘+/" data-event="showHelpDialog" tabindex="-1"><i class="icon-question"></i></button>',
            fullscreen: '<button type="button" class="btn btn-default-outline btn-sm btn-small" title="全屏" data-event="fullscreen" tabindex="-1"><i class="icon-fullscreen"></i></button>'
        }, d = '<div class="note-popover"><div class="note-link-popover popover bottom in" style="display: none;"><div class="arrow"></div><div class="popover-content note-link-content"><a href="http://www.google.com" target="_blank">www.google.com</a>&nbsp;&nbsp;<div class="note-insert btn-group"><button type="button" class="btn btn-default-outline btn-sm btn-small" title="编辑" data-event="showLinkDialog" tabindex="-1"><i class="icon-edit"></i></button><button type="button" class="btn btn-default-outline btn-sm btn-small" title="取消链接" data-event="unlink" tabindex="-1"><i class="icon-unlink"></i></button></div></div></div><div class="note-image-popover popover bottom in" style="display: none;"><div class="arrow"></div><div class="popover-content note-image-content"><div class="btn-group"><button type="button" class="btn btn-default-outline btn-sm btn-small" title="Resize Full" data-event="resize" data-value="1" tabindex="-1"><i class="icon-resize-full"></i></button><button type="button" class="btn btn-default-outline btn-sm btn-small" title="Resize Half" data-event="resize" data-value="0.5" tabindex="-1">½</button><button type="button" class="btn btn-default-outline btn-sm btn-small" title="Resize Thrid" data-event="resize" data-value="0.33" tabindex="-1">⅓</button><button type="button" class="btn btn-default-outline btn-sm btn-small" title="Resize Quarter" data-event="resize" data-value="0.25" tabindex="-1">¼</button></div><div class="btn-group"><button type="button" class="btn btn-default-outline btn-sm btn-small" title="Float Left" data-event="float" data-value="left" tabindex="-1"><i class="icon-align-left"></i></button><button type="button" class="btn btn-default-outline btn-sm btn-small" title="Float Right" data-event="float" data-value="right" tabindex="-1"><i class="icon-align-right"></i></button><button type="button" class="btn btn-default-outline btn-sm btn-small" title="Float None" data-event="float" data-value="none" tabindex="-1"><i class="icon-reorder"></i></button></div></div></div></div>', e = '<div class="note-handle"><div class="note-control-selection"><div class="note-control-selection-bg"></div><div class="note-control-holder note-control-nw"></div><div class="note-control-holder note-control-ne"></div><div class="note-control-holder note-control-sw"></div><div class="note-control-sizing note-control-se"></div><div class="note-control-selection-info"></div></div></div>', g = '<table class="note-shortcut"><thead><tr><th></th><th>Text formatting</th></tr></thead><tbody><tr><td>⌘ + B</td><td>Toggle Bold</td></tr><tr><td>⌘ + I</td><td>Toggle Italic</td></tr><tr><td>⌘ + U</td><td>Toggle Underline</td></tr><tr><td>⌘ + ⇧ + S</td><td>Toggle Strike</td></tr><tr><td>⌘ + \\</td><td>Remove Font Style</td></tr></tr></tbody></table>', h = '<table class="note-shortcut"><thead><tr><th></th><th>Action</th></tr></thead><tbody><tr><td>⌘ + Z</td><td>Undo</td></tr><tr><td>⌘ + ⇧ + Z</td><td>Redo</td></tr><tr><td>⌘ + ]</td><td>Indent</td></tr><tr><td>⌘ + [</td><td>Outdent</td></tr><tr><td>⌘ + K</td><td>Insert Link</td></tr><tr><td>⌘ + ENTER</td><td>Insert Horizontal Rule</td></tr></tbody></table>', j = '<table class="note-shortcut"><thead><tr><th></th><th>Paragraph formatting</th></tr></thead><tbody><tr><td>⌘ + ⇧ + L</td><td>Align Left</td></tr><tr><td>⌘ + ⇧ + E</td><td>Align Center</td></tr><tr><td>⌘ + ⇧ + R</td><td>Align Right</td></tr><tr><td>⌘ + ⇧ + J</td><td>Justify Full</td></tr><tr><td>⌘ + ⇧ + NUM7</td><td>Ordered List</td></tr><tr><td>⌘ + ⇧ + NUM8</td><td>Unordered List</td></tr></tbody></table>', k = '<table class="note-shortcut"><thead><tr><th></th><th>Document Style</th></tr></thead><tbody><tr><td>⌘ + NUM0</td><td>Normal Text</td></tr><tr><td>⌘ + NUM1</td><td>Heading 1</td></tr><tr><td>⌘ + NUM2</td><td>Heading 2</td></tr><tr><td>⌘ + NUM3</td><td>Heading 3</td></tr><tr><td>⌘ + NUM4</td><td>Heading 4</td></tr><tr><td>⌘ + NUM5</td><td>Heading 5</td></tr><tr><td>⌘ + NUM6</td><td>Heading 6</td></tr></tbody></table>', l = '<table class="note-shortcut-layout"><tbody><tr><td>' + h + "</td><td>" + g + "</td></tr>" + "<tr><td>" + k + "</td><td>" + j + "</td></tr>" + "</tbody>" + "</table>", m = '<div class="note-dialog"><div class="note-image-dialog modal" aria-hidden="false"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true" tabindex="-1">×</button><h4>插入图片</h4></div><div class="modal-body"><div class="row-fluid"><div class="note-dropzone span12">拖动图片到这里</div><div>或者</div><input class="note-image-input" type="file" class="note-link-url" type="text" /></div></div></div></div></div><div class="note-link-dialog modal" aria-hidden="false"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true" tabindex="-1">×</button><h4>插入链接</h4></div><div class="modal-body"><div class="row-fluid"><div class="form-group"><label>文字</label><span class="note-link-text form-control input-xlarge uneditable-input" /></div><div class="form-group"><label>链接地址</label><input class="note-link-url form-control span12" type="text" /></div></div></div><div class="modal-footer"><button href="#" class="btn btn-primary note-link-btn disabled" disabled="disabled">插入</button></div></div></div></div><div class="note-help-dialog modal fade" aria-hidden="false"><div class="modal-dialog"><div class="modal-content"><div class="modal-body"><div class="modal-background"><a class="modal-close pull-right" data-dismiss="modal" aria-hidden="true" tabindex="-1">Close</a><div class="title">Keyboard shortcuts</div>' + l + '<p class="text-center"><a href="//hackerwins.github.io/summernote/" target="_blank">Summernote v0.3</a> · <a href="//github.com/HackerWins/summernote" target="_blank">Project</a> · <a href="//github.com/HackerWins/summernote/issues" target="_blank">Issues</a></p>' + "</div>" + "</div>" + "</div>" + "</div>" + "</div>", n = function(c, d) {
        ...
        }, o = [["#000000", "#424242", "#636363", "#9C9C94", "#CEC6CE", "#EFEFEF", "#EFF7F7", "#FFFFFF"], ["#FF0000", "#FF9C00", "#FFFF00", "#00FF00", "#00FFFF", "#0000FF", "#9C00FF", "#FF00FF"], ["#F7C6CE", "#FFE7CE", "#FFEFC6", "#D6EFD6", "#CEDEE7", "#CEE7F7", "#D6D6E7", "#E7D6DE"], ["#E79C9C", "#FFC69C", "#FFE79C", "#B5D6A5", "#A5C6CE", "#9CC6EF", "#B5A5D6", "#D6A5BD"], ["#E76363", "#F7AD6B", "#FFD663", "#94BD7B", "#73A5AD", "#6BADDE", "#8C7BC6", "#C67BA5"], ["#CE0000", "#E79439", "#EFC631", "#6BA54A", "#4A7B8C", "#3984C6", "#634AA5", "#A54A7B"], ["#9C0000", "#B56308", "#BD9400", "#397B21", "#104A5A", "#085294", "#311873", "#731842"], ["#630000", "#7B3900", "#846300", "#295218", "#083139", "#003163", "#21104A", "#4A1031"]], p = function(b) {
            b.find(".note-color-palette").each(function() {
                for (var b = a(this), c = b.attr("data-target-event"), d = "", e = 0, f = o.length; f > e; e++) {
                    for (var g = o[e], h = "<div>", i = 0, j = g.length; j > i; i++) {
                        var k = g[i], l = ['<button type="button" class="note-color-btn" style="background-color:', k, ';" data-event="', c, '" data-value="', k, '" title="', k, '" data-toggle="button" tabindex="-1"></button>'].join("");
                        h += l
                    }
                    h += "</div>", d += h
                }
        ...
        this.createLayout = function(b, g, h, j) {
            if (!b.next().hasClass("note-editor")) {
                ...
                g && l.height(g), h && l.data("tabsize", h);
                var o = b.html(), q = function(b) {
                    return a("<div/>").html(b).text()
                };
                f.isTextarea(b[0]) && (o = q(o)), l.html(o), l.data("NoteHistory", new i);
                for (var r = "", s = 0, t = j.length; t > s; s++) {
                    var u = j[s];
                    r += '<div class="note-' + u[0] + ' btn-group">';
                    for (var v = 0, w = u[1].length; w > v; v++)
                        r += c[u[1][v]];
                    r += "</div>"
                }
                r = '<div class="note-toolbar btn-toolbar">' + r + "</div>";
                var x = a(r).prependTo(k);
                p(x), n(x, "bottom");
                var y = a(d).prependTo(k);
                n(y), a(e).prependTo(k), a(m).prependTo(k), k.insertAfter(b), b.hide()
            }
        };
    ...
    a.fn.extend({
        summernote: function(b) {
            if (b = a.extend({
                toolbar: [["style", ["style"]], ["font", ["bold", "italic", "underline", "clear"]], ["fontsize", ["fontsize"]], ["color", ["color"]], ["para", ["ul", "ol", "paragraph"]], ["height", ["height"]], ["table", ["table"]], ["insert", ["link", "picture"]], ["fullscreen", ["fullscreen"]], ["help", ["help"]]]
        ...
        code: function(b) {
            if (void 0 === b) {
                ...
                if (0 == c.length)
                    return;
                var d = q.layoutInfoFromHolder(c), e=!(!d ||!d.editable);
        ...
        destroy: function() {
            this.each(function(b, c) {
                var d = a(c), e = q.layoutInfoFromHolder(d);
