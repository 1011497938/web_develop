...
class  activity extends MY_Controller{
    ...
    public function view(){   
        ...
        $query = $this->db->query("SELECT * FROM `news` WHERE `condition` = 1 AND `kind` = '活动'");
        ...
        $data['results'] = $this->List_model->activity_List($config['per_page'],($data['page_num']-1) * $config['per_page']);
        

        $this->load->view('common/callhead');
        $this->load->view('common/nav');
        $this->load->view('common/slider');
        $this->load->view('activity_intro',$data);
